...
class NorthTrainViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func reload() {
        if isDownloading == false {
            ...
            stationDetail.showAlert(title: "資料還在跑～～～")
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        attributedString.setAttributes([.foregroundColor: UIColor.systemOrange, .font:UIFont(name: "Optima", size: 25.0)!], range: NSMakeRange(0, attributedString.length))
        cell.textLabel?.attributedText = attributedString
        
        let trainType = train[indexPath.row].TrainTypeName.Zh_tw.split(separator: "(")[0]
        cell.trainType_N.text = String(trainType)
        cell.endStation_N.text = "往" + train[indexPath.row].EndingStationName.Zh_tw
